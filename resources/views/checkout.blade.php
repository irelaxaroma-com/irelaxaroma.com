...
@section('script')
    ...
    <script>
        ...
        $(document).ready(function() {
            var exchange_rate = 1;
            url = "{{ env('url') }}" + '/api/client/currency?FindByCurrencyId=' + localStorage.getItem("currency");
            $.ajax({
                type: 'get',
                url: url,
                headers: {
    ...
    <script>
        ...
        function countries() {
            $.ajax({
                type: 'get',
                url: "{{ env('url') }}/api/client/country?getAllData=1",
                headers: {
                    'Authorization': 'Bearer ' + customerToken,
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
                    clientid: "{{ isset(getSetting()['client_id']) ? getSetting()['client_id'] : '' }}",
        ...
        function states() {
            ...
            $.ajax({
                type: 'get',
                url: "{{ env('url') }}/api/client/state?country_id=" + country_id + '&getAllData=1',
                headers: {
                    'Authorization': 'Bearer ' + customerToken,
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
                    clientid: "{{ isset(getSetting()['client_id']) ? getSetting()['client_id'] : '' }}",
        ...
        function isDefault(input) {
            ...
            $.ajax({
                type: 'put',
                url: "{{ env('url') }}/api/client/customer_address_book/" + id,
                data: {
                    is_default: '1',
        ...
        function tax() {
            ...
            $.ajax({
                type: 'get',
                url: "{{ env('url') }}/api/client/tax_rate?state_id=" + state_id + '&currency=' + localStorage
                    .getItem("currency"),
                headers: {
                    'Authorization': 'Bearer ' + customerToken,
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
        ...
        function shippingMethodisDefault() {
            ...
            $.ajax({
                type: 'post',
                url: "{{ env('url') }}/api/client/isDefaultShipping",
                headers: {
                    'Authorization': 'Bearer ' + customerToken,
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
                    clientid: "{{ isset(getSetting()['client_id']) ? getSetting()['client_id'] : '' }}",
    ...
    <script>
        ...
        var url = "{{ env('url') }}" + '/api/client/get-braintree-auth-token';
        ...
        //     var url = "{{ env('url') }}" + '/api/client/paytm-payment';
        //     $.ajax({
        //         type: 'post',
        //         url: url,
        //         data:{
