...
@section('script')
    <script>
        ...
        function fetchProduct() {
            var url = "{{ env('url') }}" + '/api/client/products/' + "{{ $product }}" +
        ...
        function fetchRelatedProduct() {
            var url = "{{ env('url') }}" + '/api/client/products?limit=10&getDetail=1&language_id=' + languageId +
                '&currency=' + localStorage.getItem("currency");
        ...
        function productReview() {
            ...
            var url = "{{ env('url') }}" + '/api/client/review?product_id={{ $product }}&comment=' + comment +
        ...
        function getProductReview() {
            var url = "{{ env('url') }}" + '/api/client/review?product_id={{ $product }}';
            $.ajax({
                type: 'get',
                url: url,
                headers: {
