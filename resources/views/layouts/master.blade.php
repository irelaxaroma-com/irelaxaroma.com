...
<body class="animation-s1 {{ $data['direction'] === 'rtl' ? 'bodyrtl' : '' }} ">
    ...
    <script>
        ...
        function getWishlist() {
            ...
            $.ajax({
                type: 'get',
                url: "{{ env('url') }}" + '/api/client/wishlist',
                headers: {
        ...
        function addWishlist(input) {
            ...
            $.ajax({
                type: 'post',
                url: "{{ env('url') }}" + '/api/client/wishlist?product_id=' + $(input).attr('data-id'),
                headers: {
                    'Authorization': 'Bearer ' + customerToken,
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
                    clientid: "{{ isset(getSetting()['client_id']) ? getSetting()['client_id'] : '' }}",
        ...
        function addCompare(input) {
            ...
            $.ajax({
                type: 'post',
                url: "{{ env('url') }}" + '/api/client/compare?product_id=' + $(input).attr('data-id') +
        ...
        function quiclViewData(input) {
            ...
            $.ajax({
                type: 'get',
                url: "{{ env('url') }}" + '/api/client/products/' + product_id +
        ...
        function addToCartFun(product_id, product_combination_id, cartSession, qty) {
            if (loggedIn == '1') {
                url = "{{ env('url') }}" + '/api/client/cart?session_id=' + cartSession + '&product_id=' + product_id +
                    '&qty=' + qty + '&product_combination_id=' + product_combination_id;
            } else {
                url = "{{ env('url') }}" + '/api/client/cart/guest/store?session_id=' + cartSession + '&product_id=' +
        ...
        function menuCart(cartSession) {
            if (loggedIn == '1') {
                url = "{{ env('url') }}" + '/api/client/cart?session_id=' + cartSession + '&currency=' + localStorage
                    .getItem("currency");
            } else {
                url = "{{ env('url') }}" + '/api/client/cart/guest/get?session_id=' + cartSession + '&currency=' +
        ...
        $('.log_out').click(function() {
            url = "{{ env('url') }}" + '/api/client/customer_logout';
        ...
        function cartItem(cartSession) {
            if (loggedIn == '1') {
                url = "{{ env('url') }}" + '/api/client/cart?session_id=' + cartSession + '&language_id=' + languageId +
                    '&currency=' + localStorage.getItem("currency");
            } else {
                url = "{{ env('url') }}" + '/api/client/cart/guest/get?session_id=' + cartSession + '&language_id=' +
        ...
        function removeCartItem(input) {
            ...
            if (loggedIn == '1') {
                url = "{{ env('url') }}" + '/api/client/cart/delete?session_id=' + cartSession + '&product_id=' +
                    product_id +
                    '&product_combination_id=' + product_combination_id + '&language_id=' + languageId;
            } else {
                url = "{{ env('url') }}" + '/api/client/cart/guest/delete?session_id=' + cartSession + '&product_id=' +
